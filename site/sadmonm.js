    ...
    Page: function(section, callBack, filter = null, userID = 1) {
        ...
        switch (section) {
            ...
            case 'clients':
                ...
                queries.push(['SELECT ID, Nombre FROM usuarios WHERE Acceso LIKE "%meison%" {where};', (filter.column == 'Ejecutivo' ? `Nombre LIKE '%${filter.filter}%'` : ''), 'Ejecutivo']);
            ...
            case "products":
                ...
                queries.push(['SELECT ID, Clave, Marca, Proveedor, Descripcion, VentaCC, Moneda, IVA, Categoria, Subcategoria FROM cmm_productos  WHERE Categoria = "16" OR Categoria = "14" {where} OR Proveedor = "8" AND Clave LIKE "%C";;', (!['Categoria', 'Marca'].includes(filter.column)  && filter.column != '' ? `${filter.column} LIKE '%${filter.filter}%'` : '')]);
                ...
                db.objectize(queries, true, ['ID', 'Clave Interna', 'Marca', 'Proveedor', 'Descripción', 'Precio en Call Center', 'Moneda', 'IVA'], 'productItem comm', 'Clave Interna', function(ret) {
