    ...
    Page: function(section, callBack, filter = null, userID = 1) {
        ...
        switch (section) {
            ...
            case 'clients':
                ...
                queries.push(['SELECT ID, Nombre FROM usuarios WHERE Acceso LIKE "%sales%" AND NOT Acceso LIKE "%salesu%" OR Area = "2" {where};', (filter.column == 'Ejecutivo' ? `Nombre LIKE '%${filter.filter}%'` : ''), 'Ejecutivo']);
