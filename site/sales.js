    ...
    Page: function(section, callBack, filter = null, userID = 1) {
        ...
        switch (section) {
            ...
            case "products":
                ...
                queries.push(['SELECT ID, Clave, Marca, Descripcion, VentaCC, IVA, Categoria, Subcategoria, Moneda FROM cmm_productos WHERE NOT Categoria = "16" {where};', (!['Categoria', 'Marca'].includes(filter.column)  && filter.column != '' ? `${filter.column} LIKE '%${filter.filter}%'` : '')]);
