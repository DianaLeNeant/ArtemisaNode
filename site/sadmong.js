...
module.exports = {
    ...
    Page: function(section, callBack, filter = null, userID = 1) {
        ...
        switch (section) {
            ...
            case 'autoclients':
                queries.push(['SELECT * FROM cmm_ccategorias {where};', (filter.column == 'Categoria' ? `Nombre LIKE '%${filter.filter}%'` : ''), 'Categoria']);
                queries.push(['SELECT * FROM cmm_clientes WHERE Tags LIKE "%auto%" {where};', (!['Categoria', 'Subcategoria'].includes(filter.column)  && filter.column != '' ? `${filter.column} BETWEEN '${filter.filter}'` : '')]);
                queries.push(['SELECT ID, Nombre FROM usuarios {where};', (filter.column == 'Ejecutivo' ? `Nombre LIKE '%${filter.filter}%'` : ''), 'Ejecutivo']);
                db.objectize(queries, false, ['ID', 'Razón Social', 'RFC', 'Domicilio', 'Ciudad', 'Municipio', 'Estado', 'Código Postal', 'Correo Electrónico', 'Teléfono', 'Nombre', 'Domicilio (Personal)', 'Ciudad (Personal)', 'Municipio (Personal)', 'Estado (Personal)', 'Código Postal (Personal)', 'Correo Electrónico (Personal)', 'Teléfono (Personal)', 'Categoría', 'Especialidad', 'Notas', 'Ejecutivo', 'Seguimiento', 'Tags', 'Creación'], 'clientItem norecursive', 'Nombre', function(ret) {
