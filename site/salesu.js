    ...
    Page: function(section, callBack, filter = null, userID = 1) {
        ...
        switch (section) {
            ...
            case 'events':
                ...
                db.objectize(queries, false, ['ID', 'Nombre', 'Razón social', 'E-Mail', 'Teléfono', 'Sector', 'Proyecto', 'QR', 'Fecha de Registro'], 'eventItem', 'nombre', function(ret) {
                    callBack(ret);
                });
                break;
            case 'sales':
            ...
            case "ask":
                ...
                queries.push(['SELECT ID, Nombre FROM cmm_paqueterias {where};', (filter.column == 'Paqueteria' ? `Nombre LIKE '%${filter.filter}%'` : ''), 'Paqueteria']);
                queries.push(['SELECT ID, Nombre FROM cmm_arrendadoras {where};', (filter.column == 'Arrendadora' ? `Nombre LIKE '%${filter.filter}%'` : ''), 'Arrendadora']);
                db.objectize(queries, false, ['ID', 'Cliente', 'Productos', 'Arrendadora', 'Fecha de Venta', 'Paquetería', 'Notas' ], 'askItem', 'Cliente', function(ret) {
                    ret.pre = '<button class="action" data-action="dateFilter.sales.Fecha">Filtro de fechas</button>';
                    callBack(ret);
                });
                break;
            case "products":
                ...
                queries.push(['SELECT ID, Clave, Marca, Descripcion, VentaCC, IVA, Categoria, Subcategoria, Moneda FROM cmm_productos WHERE Categoria = "14" {where};', (!['Categoria', 'Marca'].includes(filter.column)  && filter.column != '' ? `${filter.column} LIKE '%${filter.filter}%'` : '')]);
