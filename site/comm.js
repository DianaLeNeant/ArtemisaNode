    ...
    Page: function(section, callBack, filter = null, userID = 1) {
        ...
        switch (section) {
            ...
            case "products":
                ...
                queries.push(['SELECT ID, Clave, Marca, Proveedor, Descripcion, VentaCC, Moneda, IVA, Categoria, Subcategoria FROM cmm_productos {where};', (!['Categoria', 'Marca'].includes(filter.column)  && filter.column != '' ? `${filter.column} LIKE '%${filter.filter}%'` : '')]);
